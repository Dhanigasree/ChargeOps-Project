...
mongoose.connection.on("connected", () => {
  console.log("Auth service MongoDB event: connected", {
    state: connectionState[mongoose.connection.readyState]
  });
});

mongoose.connection.on("disconnected", () => {
  console.error("Auth service MongoDB event: disconnected", {
    state: connectionState[mongoose.connection.readyState]
  });
});

mongoose.connection.on("reconnected", () => {
  console.log("Auth service MongoDB event: reconnected", {
    state: connectionState[mongoose.connection.readyState]
  });
});

mongoose.connection.on("error", (error) => {
  console.error("Auth service MongoDB event: error", error.message);
});
...
export const connectDatabase = async () => {
  ...
  console.log("Auth service MongoDB configuration", {
