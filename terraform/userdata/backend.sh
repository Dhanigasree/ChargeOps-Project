...
exec > >(tee /var/log/chargeops-backend-userdata.log | logger -t chargeops-backend-userdata -s 2>/dev/console) 2>&1
...
SERVICES="auth-service user-service station-service booking-service payment-service review-service admin-service api-gateway"
